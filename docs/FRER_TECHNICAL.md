...
FRER은 **8바이트 R-TAG**를 Ethernet 프레임에 삽입하여 시퀀스 관리를 수행합니다.
...
| **Reserved** | 2 bytes | `0x0000` | 예약 필드 (향후 확장용) |
| **Sequence Number** | 2 bytes | `0x0000 ~ 0xFFFF` | 프레임 순서 번호 (최대 65535) |
| **Original EtherType** | 2 bytes | `0x0800` (IP) 등 | 원본 프로토콜 타입 |
...
#### FRER 프레임 (72 bytes, +8 bytes)
```
┌──────┬──────┬─────────┬──────────┬──────────┬─────────┬─────────┬─────┐
│ Dest │ Src  │ R-TAG   │ Reserved │ Sequence │ Original│ Payload │ FCS │
│ MAC  │ MAC  │ ET      │          │ Number   │ ET      │         │     │
│ (6B) │ (6B) │ 0xF1C1  │ (2B)     │ (2B)     │ (2B)    │ (46B)   │ (4B)│
│      │      │ (2B)    │          │          │         │         │     │
└──────┴──────┴─────────┴──────────┴──────────┴─────────┴─────────┴─────┘
...
```mermaid
graph LR
    A[송신측 A] -->|원본 프레임| S1[S1 Switch]
    S1 -->|1.스트림 식별| VR[VCAP Rules]
    VR -->|2.시퀀스 생성| SG[Sequence Generator]
    SG -->|3.R-TAG 삽입| RT[R-TAG Insertion]
    RT -->|복제| REP[Replication]
    REP -->|경로1| S2[S2 Switch]
    REP -->|경로2| S3[S3 Switch]
    S2 -->|중복 제거| S4[S4 Switch]
    S3 -->|중복 제거| S4
    S4 -->|최종 프레임| B[수신측 B]
```
...
|------------|------------------|--------------|------|------|
| **64B → 72B** | 23.44 Mbps | 24.41 Mbps | +4.1% | R-TAG 8B 추가로 프레임 증가 |
| **128B → 136B** | 47.10 Mbps | 48.97 Mbps | +4.0% | 동일 IFG, 프레임 커짐 |
| **256B → 264B** | 94.62 Mbps | 98.04 Mbps | +3.6% | 자연스러운 증가 |
| **512B → 520B** | 189.98 Mbps | 196.71 Mbps | +3.5% | - |
| **1024B → 1032B** | 378.91 Mbps | 390.64 Mbps | +3.1% | - |
| **1280B → 1288B** | 445.05 Mbps | 455.18 Mbps | +2.3% | - |

**해석:**
- 처리량 증가는 R-TAG 8바이트 추가로 프레임이 커진 자연스러운 결과
...
- ✅ **FRER 오버헤드 미미함!** (2배 복제 + 8바이트 추가에도 성능 저하 없음)
...
| **오버헤드** | R-TAG 8바이트 (프레임당 0.5~11% 증가) |
